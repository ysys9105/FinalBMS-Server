...
public String commentSelect(String keyword) {
	...
	try {
		String sql = "select b.bisbn, c.cid, b.btitle, b.bimgbase64, c.ctitle, c.cauthor, c.cdate, c.ctext from book b"+
				" join book_comment c on b.bisbn = c.bisbn where c.ctitle like ? or c.ctext like ? order by c.cid desc";
		pstmt= con.prepareStatement(sql);
		pstmt.setString(1, "%" + keyword + "%");
...
public boolean lendBook(String isbn, String cid, String date) {
	...
	try {
		...
		String sql = "update lendbook set lender = ? , ldate = ? where bisbn=?";
		pstmt= con.prepareStatement(sql);
		pstmt.setString(1, cid);
...
public String myBookList(String lender) {
	...
	String result = null;
	try {
		String sql = "select bisbn, btitle, bimgbase64, ldate from lendbook where lender = ?";
		pstmt= con.prepareStatement(sql);
		...
		while(rs.next()) {
			...
			obj.put("title", rs.getString("btitle"));
			obj.put("img", rs.getString("bimgbase64"));
			obj.put("date", rs.getString("ldate"));		
			arr.add(obj);
		}
		result = arr.toJSONString();
	} catch (Exception e) {
...
public boolean returnBook(String isbn) {
	...
	try {
		...
		String sql = "update lendbook set lender = null , ldate = null where bisbn=?";
		pstmt= con.prepareStatement(sql);
		pstmt.setString(1, isbn);
